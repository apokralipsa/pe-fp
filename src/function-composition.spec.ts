...
describe("Function composition", () => {
  ...
  it("should allow to run a single function", () => {
    expect(startsWith(mixed)(["flour", "eggs", "sugar", "chocolate"])).toEqual(
      "batter made out of flour, eggs, sugar and chocolate"
    );
  });

  it("should return errors instead of throwing them", () => {
    const coldBatter = startsWith(mixed, "ingredients")
      .andThen(is(bakedIn("cold oven")))
      .andThen(is(cooledOnDampTeaTowel));

    expect(coldBatter(["flour", "eggs", "sugar", "chocolate"])).toEqual(
  ...
  it("should allow to start with multiple arguments", () => {
    ...
    const multiplyBy = (multiplier: number) => (multiplicand: number) => multiplier * multiplicand;
